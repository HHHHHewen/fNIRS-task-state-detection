function acc = SVM_gaussian(X,Y,s)
...
% features = X(:,[index(1:s),index(1:s)+44]);
...
% features=X(:,30:88);
% features=cat(2,features,X(:,1:29));

% features=X(:,20:50);
% features=cat(2,features,X(:,1:19));
% features=cat(2,features,X(:,51:88));

% features=X(:,1:10);
% features=cat(2,features,X(:,55:70));
% features=cat(2,features,X(:,11:54));
% features=cat(2,features,X(:,71:88));

% features=X;
% % 
% SVMModel = fitcsvm(features,Y,'Standardize',true,'KernelFunction','gaussian',...
%                 'KernelScale','auto');      
...
% %     Testfeatures=TestData(:,[index(1:s)]);
    ...
    SVMModel = fitcsvm(features,TrainY,'Standardize',true,'KernelFunction','gaussian',...
                'KernelScale','auto');   
    
    TestError=loss(SVMModel,Testfeatures,TestY);
%     TestError=loss(SVMModel,TestData,TestY);
    TestAcc=1-TestError;
    acc=acc+TestAcc;
