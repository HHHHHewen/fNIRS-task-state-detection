function acc = knn(X,Y,s)
% knnmdl = fitcknn(X,Y,'NumNeighbors',5,'Standardize',1);
% CVKNNMdl = crossval(knnmdl,'Leaveout','on');
% acc = 1 - kfoldLoss(CVKNNMdl);
...
features=TrainData;
    ...
    knnmdl=fitcknn(features,TrainY,'NumNeighbors',5,'Standardize',1);  
    
    TestError=loss(knnmdl,Testfeatures,TestY);
    TestAcc=1-TestError;
    acc=acc+TestAcc;
