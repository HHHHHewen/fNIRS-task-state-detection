function acc = lda(X,Y,s)
% ldaModel = fitcdiscr(X,Y);      
% CVldaModel = crossval(ldaModel,'Leaveout','on');
% acc = 1 - kfoldLoss(CVldaModel);
...
features=TrainData;
    ...
    ldaModel=fitcdiscr(features,TrainY);      
    
    TestError=loss(ldaModel,Testfeatures,TestY);
    TestAcc=1-TestError;
    acc=acc+TestAcc;
